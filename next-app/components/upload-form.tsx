...
const UploadForm = () => {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-indigo-50 to-white p-4">
            <form onSubmit={HandleSubmit} className="bg-white shadow-xl rounded-lg p-8 w-full max-w-md border border-gray-200">
                <h2 className="text-3xl font-bold text-indigo-700 mb-6 text-center">Upload a File for Ingestion</h2>
                ...
                {url && file && (
                    <div className="mb-4 flex flex-col items-center">
                        <p className="text-sm text-gray-500 mb-2">Selected file: <span className="font-semibold">{file.name}</span></p>
                        {file.type.startsWith("image/") && (
                            <img src={url} alt="Preview" className="mt-2 max-w-xs rounded-md shadow-md border border-gray-200" />
                        )}
                    </div>
                )}
                <button
